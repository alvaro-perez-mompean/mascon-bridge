...
ハットの行だけは例外で、ボタンではなく**機器**そのものを選びます。四方向はひとまとまりだから
です。ハットを倒すか、その機器のどれかのボタンを押してください。
...
この画面では**ゲーム内での働き**も表示できます。**ゲーム** で **JR EAST Train Simulator**
を選ぶと、各行にマニュアルの機能名が並びます。マニュアルが赤字で「使用しないこと」と
している `ZL` も含みます。表示されるのはゲームの既定値で、Steam のコントローラー設定で
変更できます。**なし** を選べば、元のボタン名だけの表示に戻ります。

<p align="center">
  <img src="assets/screenshot-bindings-ja.png" alt="ボタン割り当ての画面" width="520">
</p>
...
**名前**の欄は、デバイスが名乗る名前です。空欄なら `mascon-bridge` になります。ゲームは
この名前を見ません。Steam もゲームもベンダー ID とプロダクト ID で判定するので、実機の
名前を借りる必要がないからです。

この欄があるのは、名前を見る外部ソフトのためです。たとえば BVE Trainsim の入力デバイス
プラグインには、デバイス名でプロファイルを対応づけるものがあります。その場合はここに実機の
名前を入れると一致します。次にブリッジを開始したときから反映され、`config.json` では
`"ProductString"` にあたります。
